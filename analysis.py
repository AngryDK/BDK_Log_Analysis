...
def get_event_data(id2times,player_id,fight_id,wcl_api_key):
    # Select Fight
    ...
    if "completionTime" in id2times[f_id]:
        if id2times[f_id]["completionTime"] == "Brick":
            ...
            print(f"Key: {zone_name} +{keystone_level}")
    ...
    return  event_data, start_time, end_time, excel_format
...
def check_deathstrike(end_time, deathstrike_data, bloodshield_data, player_stam, vb_start, vb_end):
    df_list = []
    index = 0
    ...
    for info in deathstrike_data:
        ...
        df_list.append({
            ...
            "BS absorb": bloodshield_data[index]["absorb"],
            "hitPointsBfore": hp_before,
            "health%before": hp_per_before,
            "health%after": hp_per_after,
            "VB Active": vb_active
        })
...
while keep_running == True:
    ...
    # Gather all data
    if use_same_log == True:
        print(fight_id)
    else:
        fight_data,fight_id,player_id,player_name = get_fight_id()
    id2times = process_fight_data(fight_data)
    event_data,start_time,end_time,excel_format  = get_event_data(id2times,player_id,fight_id,wcl_api_key)
    ...
    # Run the DS script
    check_deathstrike(end_time, deathstrike_data, bloodshield_data, player_stam, vb_start, vb_end)
